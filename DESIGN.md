# PROJECT OVERRIDES — read this before the Vercel analysis below

The document that follows is an independent analysis of **Vercel's** design system. We use it for
its **structure and discipline**, not its brand identity. Where this section conflicts with
anything below, **this section wins.**

## What we take from Vercel

- **Hairlines, not shadows.** Cards sit on the page held by a 1px border, not a drop shadow.
- **The four-step surface ladder** — page, card, inset, inverted band.
- **The spacing system and whitespace philosophy.** This is the single biggest reason their pages
  read as expensive.
- **Component discipline** — buttons, inputs, cards, navigation.
- **Restraint.** Nothing decorative that does not carry meaning.

## What we deliberately do NOT take

| Vercel | Ours | Why |
|---|---|---|
| Geist font | `Instrument Serif` headings + system sans body | Geist is on every AI-built site right now. Serif headings differentiate us instantly — all five audited ring-size competitors use plain sans. |
| Blue `#0070f3` | Brass `#8A6D3B` light / `#D9B77E` dark | We sell precision about **rings**. `ringsize.app` reached the same conclusion — their draggable ring outline is gold. Ours is contextual, not decorative. |
| Brand gradients (develop / preview / ship) | **None** | These are Vercel's marketing identity. Copying them makes us look like a Vercel clone. |
| Pink `#ff0080`, magenta, cyan, violet | **None** | Same reason. One accent, no more. |
| Pure white `#ffffff` canvas | Warm off-white `#FBFAF8` | Pure white reads clinical. A warm ground suits the jewellery register. |
| Developer-platform tone | Fine-jewellery register | Our audience is buying a ring, not deploying an app. |

## Our register

**Professional, premium, simple.** The reference is a well-made physical instrument or a
fine-jewellery site: few elements, each one considered, nothing decorative. Generous whitespace is
the main thing doing the work.
...
- Live tokens are defined in `src/styles/global.css`. **Never hardcode a hex in markup.**
- Radius: `8px` default, `12px` cards, `999px` pills. Nothing more rounded.
- Motion: `120ms` state changes, `200ms` larger, `ease-out`, `transform`/`opacity` only.
  Nothing animates on scroll. No entrance animations.

## Self-check before shipping any UI

- [ ] Would this look out of place next to a well-made physical instrument?
- [ ] Is the accent used only for interaction?
- [ ] Does it work in both themes, with no flash?
- [ ] At 375px, does anything overflow?
- [ ] Is there anything here that could be removed without loss?
