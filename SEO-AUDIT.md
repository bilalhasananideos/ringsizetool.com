...
| 7 | Trim homepage "Two ways to measure" to a summary + link to `/how-to-measure…` | P1 | Low | Low |
...
| 22 | Add the two justified FAQs to `/` (§18) | P2 | Low | Low |
| 25 | Rerun Lighthouse **Performance / SEO / Best Practices** — still unrun per STATUS.md. `og.png` 105 KB and `hero-ring-hand.webp` 135 KB are the candidates. | P2 | Low | Low |
| 28 | Fix the sitemap/canonical mismatch on the homepage: sitemap emits `https://ringsizetool.com` (no slash), canonical is `https://ringsizetool.com/` | P2 | Low | Low |
...
| **P1** | Multi-object calibration (quarter, euro, ₹5) | `/` | 1 | Medium | Medium | ringsize.app offers 8; IN+PK are 19% of GSC impressions | Day 10 |
...
| **P2** | Wide-band toggle in the tool | `/` | 1 | Low | Low | Real user need; not a page | Day 15 |
| **P2** | Add `Organization`, `WebSite`, `Service` schema | all | all | Low | Low | ringsize.app runs 7 types; this site runs 5 | Day 21 |
| **P2** | Lighthouse Performance / SEO / Best Practices | all | — | Low | Low | Never run (STATUS.md item 4) | Day 25 |
| **P2** | Sitemap/canonical trailing-slash mismatch on `/` | `/` | — | Low | Low | Sitemap says no slash, canonical says slash | Day 28 |
